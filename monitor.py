...
def discord_webhook(webhook, title, full_price, current_price, colour, image, discounted, bestseller):
    data = {
        'embeds': [{
            ...
            # 'timestamp': str(datetime.strftime("%H:%M:%S")),
            'fields': [
...
def shoe_filter(product, discounted, bestseller, wanted_phrases, unwanted_phrases, gender, max_price):
...
while True:
    ...
    for product in jordan_products:
        if shoe_filter(product, discounted= True, bestseller= True, wanted_phrases=['Air Jordan'], unwanted_phrases=['FlyEase'], gender= 'male', max_price=100):
            comparison(jordans_instock, product, jordans_channel_webhook, 'nike', start)

    for product in nike_products:
        if shoe_filter(product, discounted= True, bestseller= False, wanted_phrases=['Dunk', 'Force'], unwanted_phrases=[], gender= 'male', max_price=200):
            comparison(nike_instock, product, nike_channel_webhook, 'nike', start)
        
    for product in snkrs_products:
        if shoe_filter(product, discounted= False, bestseller= False, wanted_phrases=[''], unwanted_phrases=[], gender= 'male', max_price=500):
            comparison(snkrs_instock, product, snkrs_channel_webhook, 'snkrs', start)
        
    # Sends messages every 1 hour
    start = 0
    time.sleep(5)